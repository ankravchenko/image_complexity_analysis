...
def coarse_grain(img, depth):
    ...
    for iloop in range(1, depth):
        ...
        renormalized_pattern=ski.filters.gaussian(img, sigma=(iloop, iloop), truncate=3.5, channel_axis=-1)
        '''
...
## The next six lines are simply to make sure that the image has dimension exactly 2^L * 2^L - unfortunately, this algorithm requires your data to be first scaled to this size
    ...
    depth1 = len(img)#np.log2(len(img))  # Computes the possible depth of coarse graining using one side of the image
    depth2 = len(img[0])#np.log2(len(img[0]))  # Computes the possible depth of coarse graining using the other side of the image
